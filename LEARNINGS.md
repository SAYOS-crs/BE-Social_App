...
## 9. Dependency & ESM/CommonJS Type Compatibility

### TypeScript Version Pinning (`ts-node`)
- Never use `npm audit fix --force` without verifying the resulting `typescript` version.
- `ts-node` requires TypeScript `^5.x`. Upgrades to experimental/preview releases (e.g. `typescript@7.x`) strip `ts.sys` APIs and cause runtime crashes during bootstrap (`Cannot read properties of undefined (reading 'fileExists')`).
- Always ensure `typescript` is explicitly locked to `^5.x` in `devDependencies`.

### Modern ESM Packages in CommonJS (`file-type`, `chalk`, etc.)
- When using modern ESM-first packages in a CommonJS project:
  1. Do NOT install legacy `@types/<package>` (e.g. `@types/file-type@10.x`), as their exported APIs differ drastically from modern versions and will cause missing member errors like `Module has no exported member 'fileTypeFromBuffer'`.
  2. If TypeScript cannot resolve types under `"moduleResolution": "node10"`, define a dedicated ambient declaration in `src/types/<package>.d.ts` (e.g. `src/types/file-type.d.ts`).
  3. Ensure `tsconfig.json` contains `"typeRoots": ["./node_modules/@types", "./src/types"]` so local typings take effect without modifying package build targets.
